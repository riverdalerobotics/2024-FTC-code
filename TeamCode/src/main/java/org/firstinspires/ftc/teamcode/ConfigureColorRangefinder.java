...
@TeleOp (name = "ConfigOpMode", group = "ConfigOpMode")
@Config
public class ConfigureColorRangefinder extends LinearOpMode {
    public static double yellowMin = 180;
    public static double yellowMax = 250;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        crf.setPin0Digital(ColorRangefinder.DigitalMode.HSV, 180, 250 ); // blue
        crf.setPin0Digital(ColorRangefinder.DigitalMode.HSV, 65,80);// yellow
        crf.setPin0DigitalMaxDistance(ColorRangefinder.DigitalMode.HSV, 30); // 20mm or closer requirement

        crf.setPin1Digital(ColorRangefinder.DigitalMode.HSV, 10,30); // inverted red
        crf.setPin1Digital(ColorRangefinder.DigitalMode.HSV, 65,80); // inverted yellow
        crf.setPin1DigitalMaxDistance(ColorRangefinder.DigitalMode.HSV, 30); // 20mm or closer requirement
        crf.setPin0InvertHue();
        waitForStart();
        while(opModeIsActive()){
            float [] hsv = new float[3];
            Color.RGBToHSV(crf.emulator.red(), crf.emulator.green(), crf.emulator.blue(), hsv);
            telemetry.addData("h: ", hsv[0] + "s " + hsv[1] + "v " + hsv[2]);
            telemetry.update();
            telemetry.update();
        }
    }
